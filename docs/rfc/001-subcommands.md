# RFC-001: Расширение субкоманд CLI-утилиты express-bot
...
```bash
express-bot send message "Деплой завершён"
express-bot send message --chat-id UUID --from report.txt
echo "alert" | express-bot send message
```
...
cat archive.tar.gz | express-bot send file --chat-id UUID --filename archive.tar.gz
```
...
```bash
express-bot chats create --name "Инцидент #1234" --members "uuid1,uuid2,uuid3"
express-bot chats create --name "Релиз 2.0" --description "Обсуждение релиза" --members "uuid1"
```
...
```bash
express-bot chats members add --chat-id UUID --huids "uuid1,uuid2"
express-bot chats members remove --chat-id UUID --huids "uuid1"
```
...
```bash
express-bot message edit --chat-id UUID --sync-id MSG_UUID "Новый текст"
```
...
```bash
express-bot message delete --chat-id UUID --sync-id MSG_UUID
```
...
```bash
express-bot user search --email ivan@company.ru
express-bot user search --huid UUID
express-bot user search --ad-login ivanov
```
...
```bash
express-bot bot ping          # статус, время ответа
