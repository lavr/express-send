...
Если `api_keys` пуст и `--api-key` не передан — сервер отказывается стартовать (нельзя запускать без аутентификации).
...
- 403 если ключ не найден в списке
- В `context` кладётся `name` ключа для логирования
